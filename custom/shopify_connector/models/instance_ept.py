...
class ShopifyInstanceEpt(models.Model):
    ...
    use_graphql_api = fields.Boolean(default=False, string='Use GraphQL API ?')
    ...
    def get_total_orders(self):
        ...
        def orders_of_current_week(order_query):
            qry = order_query + """
                ...
                """
            self.env.cr.execute(qry, (self.id,))
            return self.env.cr.dictfetchall()
    ...
    def get_shipped_orders(self):
        ...
        def shipped_order_of_all_time(shipped_query):
            self.env.cr.execute(shipped_query, (self.id,))
            return self.env.cr.dictfetchall()
    ...
    def get_refund(self):
        ...
        def refund_of_all_time(refund_query):
            self.env.cr.execute(refund_query, (self.id,))
            return self.env.cr.dictfetchall()
    ...
    def refresh_webhooks(self):
        ...
        if webhook_obj:
            for webhooks in webhook_obj:
                ...
                self.env.cr.execute("DELETE FROM shopify_webhook_ept WHERE id = %s", [webhooks.id], log_exceptions=False)
