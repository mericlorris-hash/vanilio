...
from ..shopify_graphql.queries.product import ProductQueryHelper
...
class ShopifyProductDataQueue(models.Model):
    ...
    def fetch_and_create_queues_from_graphql_products(self, instance, status, import_based_on, from_date, to_date, skip_existing_product, fields=None):
        ...
        has_next_page = True
        while has_next_page:
            query = ProductQueryHelper.build_product_query(
                status, import_based_on, from_date, to_date, fields, after_cursor=cursor
            )
            result = self._execute_graphql_with_retries(client, query, base_sleep, max_retries, retry_count)
            if result is None:
                break
            self._handle_graphql_cost_throttle(result)
            batch_products = self._extract_graphql_products(result)
            if batch_products:
                product_response = self.convert_response_graphql_to_rest_ept(batch_products)
                product_queue_list += self.create_product_queues(instance, product_response, skip_existing_product)
            page_info = result.get("data", {}).get("products", {}).get("pageInfo", {})
            has_next_page = page_info.get("hasNextPage", False)
            cursor = page_info.get("endCursor", False) if has_next_page else False
            _logger.info("Processed batch of %d products, next cursor: %s", len(batch_products), cursor)
        return product_queue_list

    def _execute_graphql_with_retries(self, client, query, base_sleep, max_retries, retry_count):
        """Executes a GraphQL query with retry logic for rate limits."""
        while True:
            try:
                ...
                error_msg = str(e)
                if any(x in error_msg for x in ('Throttled', 'rate limit', '429')):
                    sleep_time = base_sleep * (2 ** retry_count)
                    _logger.warning("Shopify GraphQL rate limit hit. Sleeping for %s seconds. Error: %s", sleep_time, e)
                    time.sleep(sleep_time)
                    retry_count += 1
                    if retry_count > max_retries:
                        _logger.error("Max retries exceeded for Shopify GraphQL rate limit.")
                        return None
                    continue
                _logger.error("GraphQL product fetch error: %s", e)
                return None
    ...
    @staticmethod
    def _extract_graphql_products(result):
        """Extracts product nodes from a GraphQL result."""
        edges = result.get("data", {}).get("products", {})
    ...
    def convert_graphql_variant_ept(node, product_id):
        ...
        variant_id = ShopifyProductDataQueue.gid_to_int_ept(node["id"])
        ...
        return {
            ...
            "inventory_item_id": ShopifyProductDataQueue.gid_to_int_ept(inv_item.get("legacyResourceId")),
            ...
            "option1": node["selectedOptions"][0]["value"] if node.get("selectedOptions") and len(node["selectedOptions"]) > 0 else None,
            "option2": node["selectedOptions"][1]["value"] if node.get("selectedOptions") and len(node["selectedOptions"]) > 1 else None,
            "option3": node["selectedOptions"][2]["value"] if node.get("selectedOptions") and len(node["selectedOptions"]) > 2 else None,
            "image_id": ShopifyProductDataQueue.gid_to_int_ept(node["image"]["id"]) if node.get("image") else None,
        }
    ...
    def convert_graphql_image_ept(media_node, product_id, position):
        ...
        image_id = ShopifyProductDataQueue.gid_to_int_ept(media_node["id"])
    ...
    def convert_single_response_graphql_to_rest_ept(self, product_res):
        """Main function to convert GraphQL product → REST-like dict."""
        product_id = ShopifyProductDataQueue.gid_to_int_ept(product_res["id"])
        ...
        for opt in product_res.get("options", []):
            rest_product["options"].append({
                "id": ShopifyProductDataQueue.gid_to_int_ept(opt["id"]),
                "product_id": product_id,
                "name": opt["name"],
                "position": opt["position"],
                "values": opt.get("values", []),
            })
        variants = product_res.get("variants", {}).get("edges", [])
        image_v_id = {}
        for v in variants:
            node = v["node"]
            variant_image = node.get("image", {})
            if variant_image:
                variant_id = ShopifyProductDataQueue.gid_to_int_ept(node["id"])
                image_value_id = ShopifyProductDataQueue.get_v_value(variant_image.get("src"))
                image_v_id[image_value_id] = variant_id
            rest_product["variants"].append(
                ShopifyProductDataQueue.convert_graphql_variant_ept(node, product_id)
            )
        media_edges = product_res.get("media", {}).get("edges", [])
        images = []
        for pos, m in enumerate(media_edges, start=1):
            img_dict = ShopifyProductDataQueue.convert_graphql_image_ept(m["node"], product_id, pos)
            image_value_id = ShopifyProductDataQueue.get_v_value(img_dict.get("src"))
            if image_value_id in image_v_id:
                img_dict["variant_ids"].append(image_v_id[image_value_id])
